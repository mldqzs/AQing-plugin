...
class Setting {
  constructor () {
    /** 默认设置 */
    this.defPath = `${_path}/plugins/AQing-plugin/config/defSet_config/`
    this.defSet = {}

    /** 用户设置 */
    this.configPath = `${_path}/plugins/AQing-plugin/config/config/`
    this.config = {}
  ...
  watch (file, app, type = 'defSet') {
    ...
    watcher.on('change', path => {
      ...
      logger.mark(`[AQing-plugin][修改配置文件][${type}][${app}]`)
      if (this[`change_${app}`]) {
        this[`change_${app}`]()
      }
    })
