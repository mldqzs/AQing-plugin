...
- QQ音乐需在锅巴「AQing-plugin → 音乐解析」手动填写已登录网页的完整 Cookie；酷狗音乐推荐由主人发送 `#酷狗音乐扫码登录`，机器人会发送官方登录二维码并自动保存登录信息，`退出登录` 命令可清除；
- 酷狗也支持手动填写完整 Cookie，或只填写核心 `KugooID` 和 Token；歌词与音源权限分开解析，即使 VIP 音频没有权限也会尽量返回完整歌词；
